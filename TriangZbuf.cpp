...
img::EasyImage triangZbuf(const ini::Configuration& configuration){
    ...
    if (type == "LightedZBuffering"){
        ...
        for (int i = 0; i < nrl; i++){
            ...
            auto al = configuration[lightname]["ambientLight"].as_double_tuple_or_die();
            Color ambient(al[0], al[1], al[2]);
            light.ambientLight = ambient;
            auto sl = configuration[lightname]["specularLight"].as_double_tuple_or_default(defa);
            auto dl = configuration[lightname]["diffuseLight"].as_double_tuple_or_default(defa);
            if (sl.size() > 1 or dl.size() > 1){
                extralight = true;
            }
    ...
    if (!extralight) {
        for (int i = 0; i < nrFigures; i++) {
            ...
            if (type2 == "LineDrawing") {
                ...
                auto rotatZ = configuration[figure_name]["rotateZ"].as_double_or_die();
                auto scal = configuration[figure_name]["scale"].as_double_or_die();
                auto nrit = configuration[figure_name]["nrIterations"].as_int_or_die();
                auto fscale = configuration[figure_name]["fractalScale"].as_double_or_die();
                ...
                Figures3D fractal;
                generateFractal(cube, fractal, nrit, fscale);
                for (auto &f: fractal) {
